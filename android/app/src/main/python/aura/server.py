...
from . import catalog, config, host
from . import desktop
...
def _default_webui_dir() -> Path:
    """Where the single-page interface lives.

    Normally it sits next to this file. The Android build passes its own
    directory in (AURA_WEBUI_DIR), which is what makes the same interface work
    inside the phone app without a second copy of the markup.
    """
    override = host.webui_dir()
    if override and (Path(override) / "index.html").is_file():
        return Path(override).resolve()
    return Path(__file__).resolve().parent / "webui"


WEBUI_DIR = _default_webui_dir()
MAX_BODY = 512 * 1024 * 1024


def set_webui_dir(path) -> Path:
    """Point the server at a different interface folder (used by the tests)."""
    global WEBUI_DIR
    WEBUI_DIR = Path(path).resolve()
    return WEBUI_DIR
...
class Api:
    ...
    def host_payload(self) -> dict:
        """Which machine this is, and what it is allowed to do.

        The interface uses this to talk about the app in the right terms ("this
        phone" rather than "this machine"), to offer the file picker only where
        there is one, and to say plainly when AURA has no access to the user's
        files. `bundled_engine` is True for the Android build, where the model
        engine came with the app instead of being downloaded.
        """
        engine = self.engine.engine_state()
        payload = host.describe(bundled_engine=bool(engine.get("bundled")))
        payload.update({
            "data_dir": str(self.library.root),
            "models_dir": str(self.engine.models_dir()),
            "models_dir_default": str(config.models_dir_for(self.library.root, {})),
            "document_count": len(self.library.documents),
            "engine": {
                "installed": bool(engine.get("installed")),
                "bundled": bool(engine.get("bundled")),
                "complete": bool(engine.get("complete")),
                "missing": list(engine.get("missing") or []),
                "folder": engine.get("folder") or "",
                "tag": engine.get("tag") or "",
            },
        })
        return payload

    def pick(self, kind: str = "files") -> dict:
        """Refuse politely: this build shows no system file picker.

        The Android app has one, but it is reached from the interface itself
        (the WebView's JavaScript bridge) rather than through Python, so the
        code that runs on both platforms does not have to know about it.
        """
        raise AuraError("this build cannot open a file picker - type or paste the path "
                        "instead")
    ...
    def update_settings(self, payload: dict) -> dict:
        ...
        if "models_dir" in payload:
            payload["models_dir"] = self._check_models_dir(payload.get("models_dir"))
    ...
    def _check_models_dir(self, value) -> str:
        """Accept a folder for models only if AURA can really write into it.

        A phone user picks this folder by hand, so getting a "yes" here and a
        permission error three gigabytes later would be the worst possible
        outcome; the empty string means "use the default".
        """
        ...
        if target.exists() and not target.is_dir():
            raise AuraError("{} is a file, not a folder".format(target))
        try:
            target.mkdir(parents=True, exist_ok=True)
            probe = target / ".aura-write-test"
            probe.write_bytes(b"")
            probe.unlink()
        except OSError as exc:
            raise AuraError("AURA cannot write to {} ({}). Pick another folder, or grant "
                            "access to files in the app's own settings.".format(target, exc))
        return str(target)
    ...
    def models_payload(self) -> dict:
        ...
        return {
            ...
            "settings": {key: settings.get(key) for key in (
                ...
                "llm_threads", "llm_server_url", "models_dir")},
            "platform": {"key": catalog.platform_key(), "arch": catalog.arch_key(),
                         "asset": catalog.runtime_pattern()},
            # where a model would be written, and whether the engine even exists
            # on this machine (on a phone it came with the app)
            "folder": {"path": str(folder), "default": str(config.models_dir_for(self.library.root, {})),
                       "chosen": bool(str(settings.get("models_dir") or "")),
                       "writable": _writable(folder)},
            "bundle": catalog.bundled_engine_report() if catalog.is_bundled_engine() else None,
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        try:
            ...
            if route == "/api/models":
                ...
                return
            if route == "/api/host":
                self._send_json(self.api.host_payload())
                return
            if route == "/api/jobs":
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        try:
            ...
            if route == "/api/pick":
                payload = self._decode_json(body)
                self._send_json(self.api.pick(payload.get("kind", "files")))
                return
            if route == "/api/upload":
...
def _writable(folder) -> bool:
    """Can AURA actually write here? (The model folder is chosen by the user.)"""
    try:
        probe = Path(folder) / ".aura-write-test"
        probe.write_bytes(b"")
        probe.unlink()
        return True
    except OSError:
